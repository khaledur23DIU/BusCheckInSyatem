@extends('layouts.app', ['activePage' => 'editAssignChecker', 'titlePage' => __('Assign Checker Manageement')])
...
@section('content')
  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-4">
        <form id="assignCheckerValidation" action="{{ route('assignChecker.update',$assignedChecker->id) }}" method="POST" enctype="multipart/form-data">
          @csrf
          @method('PUT')
          <div class="card ">
            ...
            <div class="card-body">
              <div class="row">
                ...
                <select class="selectpicker @error('bus_route') is-invalid @enderror" id="bus_route" name="bus_route" data-style="select-with-transition" title="{{__('Bus Route')}}" required="true">
                  ...
                  <option {{$assignedChecker->checkingPlace->busRoute->id == $route->id ? 'selected':''}} value="{{$route->id}}">{{$route->departure_starting_place.'-'.$route->departure_ending_place.'-'.$route->departure_starting_place}} </option>
                  @endforeach
                </select>
                @error('bus_route')
              ...
              <div class="row">
                ...
                <select class="selectpicker busRouteType @error('bus_route_type') is-invalid @enderror" id="busRouteType" name="bus_route_type" data-style="select-with-transition" title="{{__('Select One')}}" required="true">
                  <option {{$assignedChecker->checkingPlace->bus_route_type == 1 ? 'selected' :''}} value="1"> {{__('Departure')}} </option>
                  <option {{$assignedChecker->checkingPlace->bus_route_type == 2 ? 'selected' :''}} value="2"> {{__('Return')}} </option>
                </select>
              ...
              <div class="row">
                ...
                <select class="selectpicker checkInPlace @error('check_in_place') is-invalid @enderror" id="checkInPlace" name="check_in_place" data-style="select-with-transition" title="{{__('Select One')}}" required="true">
                  @foreach ($assignedChecker->checkingPlace->busRoute->busStops as $busStop)
                  <option {{$assignedChecker->bus_stop_id == $busStop->id ?'selected':''}} value="{{$busStop->id}}">{{$busStop->bus_stop}}</option>
                  @endforeach
                </select>
              ...
              <div class="row">
                ...
                <select class="selectpicker @error('checker') is-invalid @enderror" id="checker" name="checker" data-style="select-with-transition" title="{{__('Checker')}}" required="true">
                  ...
                  <option {{$assignedChecker->checker_id == $checker->id ? 'selected':''}} value="{{$checker->id}}">{{$checker->name}} </option>
                  @endforeach
                </select>
                @error('checker')
                <label class="error">{{ __($message) }}</label> 
            ...
            <div class="card-footer ml-auto mr-auto">
              ...
              <a href="{{ route('assignChecker.index') }}" class="btn btn-rose">{{__('Cancel')}}</a>
            </div>
          </div>
        </form>
      </div>
