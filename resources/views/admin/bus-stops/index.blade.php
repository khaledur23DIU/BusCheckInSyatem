@extends('layouts.app', ['activePage' => 'busStop', 'titlePage' => __('Bus Stops Management')])
...
@section('content')
  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-4">
        <form id="busStopsValidation" action="{{ route('busStop.store') }}" method="POST" enctype="multipart/form-data">
          @csrf
          <div class="card ">
            ...
            <div class="card-body">
              ...
              <div class="row">
                ...
                <select class="selectpicker @error('bus_route_type') is-invalid @enderror" id="bus_route_type" name="bus_route_type" data-style="select-with-transition" title="{{__('Bus Route Type')}}" required="true">
                  <option disabled> {{__('Select One')}}</option>
                  <option value="1">{{__('Departure')}}</option>
                  <option value="2">{{__('Return')}}</option>
                </select>
              ...
              <div class="row">
                ...
                <div class="col-sm-6">
                  <div class="form-group">
                    <input type="text" class="form-control @error('bus_stop') is-invalid @enderror" placeholder="Bus Stop" id="bus_stop" name="bus_stop" required="true" value="{{old('bus_stop')}}">
                    @error('bus_stop')
                    <label class="error">{{ __($message) }}</label> 
                    @enderror
                  </div>
