@extends('layouts.app', ['activePage' => 'editBusRoute', 'titlePage' => __('Bus Route Management')])
...
@section('content')
  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-5">
        <form id="busRouteValidation" action="{{ route('busRoute.update',$busRoute->id) }}" method="POST" enctype="multipart/form-data">
          @csrf
          @method('PUT')
          <div class="card ">
            ...
            <div class="card-body ">
              <div class="form-group">
                ...
                <input type="text" class="form-control @error('departure_starting_place') is-invalid @enderror" maxLength="20" id="departure_starting_place" name="departure_starting_place" value="{{$busRoute->departure_starting_place}}" required="true" autofocus="departure_starting_place">
              ...
              <div class="form-group">
                ...
                <input type="text" class="form-control @error('departure_ending_place') is-invalid @enderror" maxLength="20" id="departure_ending_place" name="departure_ending_place" value="{{$busRoute->departure_ending_place}}" required="true" autofocus="departure_ending_place">
              ...
              <div class="form-group">
                ...
                <input type="text" class="form-control" maxLength="20" readonly="true" disabled="true" id="return_starting_place" name="return_starting_place" value="{{$busRoute->departure_ending_place}}" autofocus="return_starting_place">
              </div>
              <div class="form-group">
                <label for="return_ending_place" class="bmd-label-floating"> {{__('Return Ending Place')}} *</label>
                <input type="text" class="form-control" maxLength="20" readonly="true" disabled="true" id="return_ending_place" name="return_ending_place" value="{{$busRoute->departure_starting_place}}" autofocus="return_ending_place">
              </div>
              <div class="row">
                <div class="col-md-6">
                <select class="bmd-label-floating selectpicker @error('status') is-invalid @enderror" id="status" name="status" data-style="select-with-transition" title="{{__('Is Route Active? *')}}" required="true">
                  ...
                  <option {{$busRoute->is_active == 1 ? 'selected':''}} value="1">{{('Yes')}} </option>
                  <option {{$busRoute->is_active == 0 ? 'selected':''}} value="0">{{('No')}}</option>
                </select>
                @error('status')
                <label class="error">{{ __($message) }}</label> 
                @enderror
            ...
            <div class="card-footer ml-auto mr">
              ...
              <a href="{{ route('busRoute.index') }}" class="btn btn-rose">{{__('Cancel')}}</a>
            </div>
          </div>
        </form>
      </div>
