@extends('layouts.app', ['activePage' => 'editBus', 'titlePage' => __('Bus Management')])
...
@section('content')
  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-5">
        <form id="busValidation" action="{{ route('bus.update',$bus->id) }}" method="POST" enctype="multipart/form-data">
          @csrf
          @method('PUT')
          <div class="card ">
            ...
            <div class="card-body ">
              <div class="form-group">
                ...
                <input type="text" class="form-control @error('bus_no') is-invalid @enderror" id="bus_no" name="bus_no" value="{{$bus->bus_no}}" required="true" autofocus="bus_no">
                @error('bus_no')
              ...
              <div class="row">
                ...
                <select class="bmd-label-floating selectpicker @error('status') is-invalid @enderror" id="status" name="status" data-style="select-with-transition" title="{{__('Is Bus Running? *')}}" required="true">
                  ...
                  <option {{ $bus->is_running == 1 ? 'selected':'' }} value="1">{{('Yes')}} </option>
                  <option {{ $bus->is_running == 0 ? 'selected':'' }} value="0">{{('No')}}</option>
                </select>
                @error('status')
                <label class="error">{{ __($message) }}</label> 
                @enderror
            ...
            <div class="card-footer ml-auto mr">
              ...
              <a href="{{ route('bus.index') }}" class="btn btn-rose">{{__('Cancel')}}</a>
            </div>
          </div>
        </form>
      </div>
